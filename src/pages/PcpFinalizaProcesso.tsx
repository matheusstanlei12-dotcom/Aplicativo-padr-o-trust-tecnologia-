...
export const PcpFinalizaProcesso: React.FC = () => {
    ...
    const filtered = peritagens.filter(p => {
        const cMatch = p.cliente ? p.cliente.toLowerCase() : '';
        const nMatch = p.numero_peritagem ? p.numero_peritagem.toLowerCase() : '';
        const osMatch = p.os_interna ? p.os_interna.toLowerCase() : '';
        const term = searchTerm.toLowerCase();
        
        return cMatch.includes(term) || nMatch.includes(term) || osMatch.includes(term);
    });
